...
echo "EXTREMA settlement-before-observation-end fork smoke test"
...
if (( TARGET_TS >= OBS_END )); then
  echo "ERROR: invalid smoke window: target timestamp is not before observation end" >&2
  exit 1
fi
...
anvil   --fork-url "$UPSTREAM_RPC"   --port "$PORT"   --chain-id 5042002   --silent   > /tmp/extrema-settlement-before-end-anvil.log 2>&1 &
ANVIL_PID=$!
...
if (( BLOCK_TS <= ENTRY_CLOSE || BLOCK_TS >= OBS_END )); then
  echo "ERROR: fork timestamp is not inside the required post-close/pre-observation-end window" >&2
  exit 1
fi
...
LOCKED_STATE="$(cast call "$POOL"   "getRound(uint256)((uint64,uint64,uint64,uint64,uint8,uint64,uint64,uint256,uint256,uint64,uint256[3]))"   "$ROUND_ID"   --rpc-url "$LOCAL_RPC")"

echo "Local fork round state after lock:"
echo "$LOCKED_STATE"

echo
echo "Read-only resolver settle call before observation end (expected ObservationNotEnded):"
set +e
...
if [[ $EXIT_CODE -eq 0 ]]; then
  echo "ERROR: early settlement unexpectedly succeeded on fork" >&2
  exit 1
fi

if [[ "$OUT" != *"ObservationNotEnded"* ]]; then
  echo "ERROR: expected ObservationNotEnded revert, got a different failure" >&2
  exit 1
fi

echo
echo "SETTLEMENT_BEFORE_END_FORK_SMOKE=PASS"
