...
Everything newly generalized in this correction is **IMPLEMENTED / DETERMINISTICALLY VALIDATED, NOT LIVE PROVEN**. The limited Arbitrum approval evidence for the one preserved action recorded further down is live proven; its deposit submission and Gateway credit/finality are not.
...
- While a deposit is in flight, the one form remains bound to that source and shows its finality rail, so a second deposit is never started from another chain automatically or otherwise. The backend returns the authoritative recovery disposition: `COMPLETED`, and `FAILED` or `EXPIRED` rows with no durable financial evidence, return `CLEAR`; only then does the browser retire local recovery and unlock a manual new deposit. `RECONCILING`, `DEPOSIT_PENDING`, `RECONCILIATION_REQUIRED`, and every `FAILED` or `EXPIRED` row with evidence return `RECONCILE`, preserve the lock, and continue status-only review. No state triggers a blind retry or deletes the durable action.
...
- Deterministic validation evidence: `GATEWAY_WALLET_UI=PASS`, `GATEWAY_SOURCE_FORM_UI=PASS`, `GATEWAY_SELECTED_SOURCE_BALANCE_UI=PASS`, `GATEWAY_DEPOSIT_RECONCILIATION_REQUIRED_PRESERVED=PASS`, `GATEWAY_UNCERTAIN_RECOVERY_FAILS_CLOSED=PASS`, `GATEWAY_CLEAN_TERMINAL_RECOVERY_RELEASE=PASS`, `GATEWAY_POINTER_FOCUS=PASS`, `GATEWAY_HEADER_ACCOUNT_FOOTPRINT=PASS`, `GATEWAY_SESSION_CONTROLS=PASS`, `GATEWAY_DEPOSIT_LIVE_NETWORK_CALLS=0`, `WALLET_PAGE_DEPOSIT_RECOVERY_LIVE_NETWORK_CALLS=0`, `LIVE_GATEWAY_BROADCAST=NOT_EXECUTED`.
...
- Current backend state: `DEPOSIT_CHALLENGE`. Approval challenge `eb639625-47fd-5b48-ad6c-1fda94fd7cb5` has Circle transaction `4e7e6522-291e-5799-9a14-5d4efd4209d9` and source-chain approval transaction `0x0d4ad6ec3fe6c02adb6933fc55443aabde1e987f00f69627e244f34e7ad2b964`.
- The existing deposit challenge is `5248643d-fe9c-52da-b1ab-c9589f3b47f5`; its Circle transaction id and deposit tx hash are still `null`. Source balance/allowance were `20000000` / `1000000`; Gateway domain-3 balance was `0`.
- Boundary: **Arbitrum approval is LIVE PROVEN. Arbitrum deposit submission and Gateway credit/finality are NOT LIVE PROVEN.** This exact action must be resumed; no replacement action or second approval/deposit challenge is authorized.
...
- [x] Circle Gateway deposit polling made phase-aware and recovery synchronization implemented
  ...
  - Fix: the shipped `gateway-actions.ts` poll helper now accepts a phase stop predicate. Approval polling stops on any state other than `APPROVAL_CHALLENGE`; deposit polling stops on any state other than `DEPOSIT_CHALLENGE`. The same action’s recovery is persisted as `DEPOSIT_CHALLENGE` with the backend’s existing `depositChallengeId`, then `DEPOSIT_PENDING`, then `RECONCILING`; a resumed `APPROVAL_PENDING` recovery is advanced before executing any challenge.
  ...
  - Live boundary: this correction resumes the preserved Arbitrum action above but does not execute its `DEPOSIT_CHALLENGE`; Arbitrum deposit submission and Gateway credit/finality remain **NOT LIVE PROVEN**.
...
- Production facts, unchanged by this work: Base Sepolia source deposit remains **LIVE PROVEN** (recorded above); the preserved Arbitrum action has **LIVE PROVEN approval only**, while its deposit submission and Gateway credit/finality remain **OPEN / NOT LIVE PROVEN**; generalized OP and Ethereum Sepolia source deposits and every Gateway transfer remain **OPEN / NOT LIVE PROVEN** until an explicitly approved real production proof.
