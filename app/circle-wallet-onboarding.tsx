...
type CircleSdk = {
  ...
  execute(
    challengeId: string,
    onCompleted?: (error: { message: string } | undefined) => void,
  ): void;
};
...
export function CircleWalletOnboarding({
  ...
  const socialProviderRef = useRef<unknown>("Google");
  ...
  async function bindExtremaSession(auth: CircleLoginResult, sdk: CircleSdk) {
    ...
    const initialized = await backendApi.circle.initializeWallet(
      auth.userToken,
      crypto.randomUUID(),
    );
    ...
    const session = await createCircleSessionWhenIndexed(
      auth.userToken,
      auth.refreshToken
        ? { refreshToken: auth.refreshToken, deviceId }
        : undefined,
    );
              ...
              ...(googleClientId
                ? {
                    google: {
                      ...
                      redirectUri:
                        process.env.NEXT_PUBLIC_CIRCLE_GOOGLE_REDIRECT_URI ||
  ...
  async function continueWithGoogle() {
    ...
    try {
      ...
      const deviceId = await sdk.getDeviceId();
      const device = await backendApi.circle.socialDeviceToken(
        deviceId,
        crypto.randomUUID(),
      );
    ...
    <div className="ex-circle-onboarding">
      <div className="ex-wallet-panel__actions">
        <button
          ...
          disabled={!googleClientId || Boolean(busy)}
        >
          Continue with Google
        </button>
      </div>
      ...
      <p className="ex-wallet-panel__note">
        {busy ||
          "Circle hosts Google authentication and transaction approval; EXTREMA never receives your private key."}
      </p>
